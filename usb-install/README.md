...
battery at least 15%, free space for the backup, and the backup itself is
...
1. **Patch**: Magisk then Install then *Select and Patch a File* then pick
   `determination-boot.img` from the USB drive. Output appears as
   `/sdcard/Download/magisk_patched-XXXXX.img`.
2. **Flash**: Magisk then Modules then *Install from storage* then pick
...
Magisk then Modules then Install from storage then `determination-kernel-restore.zip`.
