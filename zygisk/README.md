# DecemberOS Zygisk module — milestone 6, deliberately last
...
1. **SF-death handler suppression** — replace the `desktop-on` suppressor loop:
   ...
   issuance so a stopped SF *stays* stopped while `/data/decemberos/run/desktop-mode`
...
3. **Summon UX** — QS tile / gesture in SystemUI wired to
   `/data/decemberos/bin/desktop-on|off`, as a hook, not a replaced APK.
