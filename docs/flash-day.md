...
- [x] `boot/decemberos-boot.img` — real installed boot_b
      (`artifacts/boot_b-crdroid-12.10.img`) repacked with our kernel;
      kernel-inside verified as our build, cmdline preserved
- [x] Magisk module zip (`magisk-module/decemberos-magisk-*.zip`) with static
      aarch64 evgrab + toggle scripts payload
- [x] Host toolkit: `./dos` helper, magiskboot + cross tools in `toolchain/`
...
2. **Magisk-patch the boot image** (on the phone, no cable needed):
   `./dos connect && adb push boot/decemberos-boot.img /sdcard/Download/`
...
4. **Verify while RAM-booted** (`./dos status` — wants: root ok, our uname,
...
6. **Install the module**: `./dos push-module`, reboot, `./dos status` again.
7. Proceed to guest bring-up: `guest/build-rootfs.sh` (needs mmdebstrap on
   the host) + a static arm64 lxc for `/data/decemberos/lxc/bin`.
